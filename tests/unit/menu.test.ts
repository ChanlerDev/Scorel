...
describe("buildAppMenu", () => {
  ...
  it("builds a menu with the expected keyboard shortcuts", () => {
    ...
    expect(fileMenu?.submenu?.some((item) => item.label === "Settings" && item.accelerator === "CmdOrCtrl+," )).toBe(true);
  });
  ...
  it("wires the Settings menu item to the renderer event", () => {
    const send = vi.fn();
    buildAppMenu(() => ({ webContents: { send } } as never));
    ...
    expect(send).toHaveBeenCalledWith("menu:settings");
  });
